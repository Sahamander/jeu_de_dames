...
import atelier1.gui.GuiFactory;
...
public class Controller implements Mediator, BoardGame<Integer>, EventHandler<MouseEvent>  {
	...
	public OutputModelData<Integer> moveCapturePromote(Integer toMovePieceIndex, Integer targetSquareIndex) {
		...
		OutputModelData<Coord> ControllerDataRetour = null;
		Coord movePieceToIndex = transformIndexToCoord(toMovePieceIndex);
		Coord squareTargetIndex = transformIndexToCoord(targetSquareIndex);
		Integer promotedPieceIndex = null, capturedPieceIndex =null;
		PieceSquareColor promotedPieceColor = null;
		InputViewData<Integer> inputViewEnvoi = null;

		ControllerDataRetour = model.moveCapturePromote(movePieceToIndex,squareTargetIndex);
		promotedPieceIndex = transformCoordToIndex(ControllerDataRetour.promotedPieceCoord);
		promotedPieceColor = ControllerDataRetour.promotedPieceColor;
		capturedPieceIndex = transformCoordToIndex(ControllerDataRetour.capturedPieceCoord);

		inputViewEnvoi = new InputViewData<Integer>(toMovePieceIndex,targetSquareIndex,capturedPieceIndex,promotedPieceIndex,promotedPieceColor);

		if(ControllerDataRetour.isMoveDone){view.actionOnGui(inputViewEnvoi);}
